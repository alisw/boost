...
class GeneratorContext extends EventEmitter {
  ...
  /**
   * Requires the module specified by request within the GeneratorContext module first, then using the specified require
   * option, which defaults to the global require function.
   */
  require (request, opts = {}) {
    try {
      return this.module.require(request)
    } catch {
      return (opts.require ?? require)(request)
    }
  }
  ...
  _registerExtensions (playbook, vars) {
    ...
    if (extensions.length) {
      ...
      extensions.forEach((ext) => {
        ...
        if (!enabled) return
        if (request == null) {
          this.getLogger().warn('Skipping Antora extension entry because require key not specified')
          return
        }
        let register
        try {
